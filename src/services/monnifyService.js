...
async function getAccessToken() {
  ...
  const { data } = await axios.post(`${baseUrl}/api/v1/auth/login`, {}, { headers: { Authorization: `Basic ${basicAuth}` } });
  if (!data?.requestSuccessful) throw new Error(`Monnify auth failed: ${data?.responseMessage || 'unknown error'}`);
...
export async function initOneTimeTransaction({ amount, customerName, customerEmail, paymentReference, paymentDescription, redirectUrl, splitConfig }) {
  ...
  const payload = { amount, customerName, customerEmail, paymentReference, paymentDescription, currencyCode: 'NGN', contractCode, redirectUrl, paymentMethods: ['CARD', 'ACCOUNT_TRANSFER', 'USSD'] };
  if (splitConfig?.length) payload.incomeSplitConfig = splitConfig;
...
export async function createReservedAccount({ accountReference, accountName, customerName, customerEmail, bvn, getAllAvailableBanks = true, incomeSplitConfig = [], restrictPaymentSource }) {
  ...
  if (incomeSplitConfig.length) payload.incomeSplitConfig = incomeSplitConfig;
