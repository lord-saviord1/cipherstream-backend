...
router.post('/monnify', async (req, res) => {
  try {
    ...
    if (!signature || !verifyWebhookSignature(rawBody, signature)) {
      console.warn('[webhook] Invalid Monnify signature — rejecting');
    ...
    if (!transaction) {
      console.warn(`[webhook] No matching transaction for reference ${paymentReference}`);
    ...
    if (transaction.purpose === 'individual_unlock') {
      ...
      await Business.findByIdAndUpdate(transaction.accountId, {
        subscriptionActive: true, $addToSet: { subscriptionBundles: { $each: bundleIds } }, currentBillingPeriodEnd: periodEnd,
      });
    }
