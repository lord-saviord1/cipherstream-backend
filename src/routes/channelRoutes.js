...
router.get('/', async (req, res) => {
  try {
    ...
    const channels = await Channel.find(filter).select('-streamUrl -sourceGroup').sort({ category: 1, displayName: 1 });
    res.json({ channels });
  } catch (err) {
    console.error(err);
...
router.get('/:id/stream', requireAuth, async (req, res) => {
  try {
    ...
    // Launch phase: no paywall. Any signed-in user/business gets every channel.
    if (config.freeAccessMode) {
      return res.json({ streamUrl: channel.streamUrl });
    }
