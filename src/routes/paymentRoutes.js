...
router.post('/unlock', requireAuth, async (req, res) => {
  try {
    ...
    const transaction = await Transaction.create({
      paymentReference, accountType: 'user', accountId: user._id, accountModelName: 'User',
      bundleId, amount: bundle.priceIndividual, purpose: 'individual_unlock', status: 'PENDING',
    });

    const monnifyResponse = await initOneTimeTransaction({
      amount: bundle.priceIndividual, customerName: user.name, customerEmail: user.email,
      paymentReference, paymentDescription: `CipherStream unlock: ${bundle.name}`,
      redirectUrl: `${config.frontendUrl}/unlock/callback?ref=${paymentReference}`,
    });
...
router.post('/business/subscribe', requireAuth, async (req, res) => {
  try {
    ...
    const transaction = await Transaction.create({
      paymentReference, accountType: 'business', accountId: business._id, accountModelName: 'Business',
      bundleId: bundleIds.join(','), amount: totalMonthly, purpose: 'business_subscription_payment', status: 'PENDING',
    });

    const monnifyResponse = await initOneTimeTransaction({
      amount: totalMonthly, customerName: business.businessName, customerEmail: business.contactEmail,
      paymentReference, paymentDescription: `CipherStream Business subscription: ${bundleIds.join(', ')}`,
      redirectUrl: `${config.frontendUrl}/business/subscribe/callback?ref=${paymentReference}`,
    });

    res.json({ checkoutUrl: monnifyResponse.checkoutUrl, paymentReference, transactionId: transaction._id, pendingBundles: bundleIds });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Failed to initiate business subscription' });
  }
});
