...
function required(name, fallback = undefined) {
  ...
  if (value === undefined) {
    console.warn(`[config] Missing env var: ${name} (app may fail when this is used)`);
  }
  return value;
}
...
export const config = {
  ...
  // Launch phase: everything free, no Monnify paywall. Flip to false once
  // monetization starts — that's the only code change needed to re-enable
  // gating; Monnify service/routes are left intact underneath.
